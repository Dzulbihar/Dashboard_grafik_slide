        ...
        <div class="card">
          <div class="card-header">
            <figure class="highcharts-figure">
              <div id="grafik_departure_teus_international"></div>
            </figure>
          </div>
        </div>
      </div>
...
Highcharts.chart('grafik_departure_teus_international', {
    chart: {
        type: 'column',
        options3d: {
          enabled: true,
    ...
    title: {
        text: 
        'Grafik Departure TEUS International' 
    },
    subtitle: {
        ...
        data:
            ...
            <?php 
              $query2 = oci_parse($koneksi,     
                " 
                SELECT SUM(JML_TEUS) AS JML_TEUS, BULAN_DEPARTURE FROM DASHBOARDGRAFIK.S_PROD_PEND_PERCUSTOMER 
                  WHERE TAHUN=$tahun_lalu AND BULAN_DEPARTURE<=$bulan AND LOKASI='INT'
        ...
        data:
            ...
            <?php 
              $query2 = oci_parse($koneksi,     
                " 
                SELECT SUM(JML_TEUS) AS JML_TEUS, BULAN_DEPARTURE FROM DASHBOARDGRAFIK.S_PROD_PEND_PERCUSTOMER 
                  WHERE TAHUN=$tahun_ini AND BULAN_DEPARTURE<=$bulan AND LOKASI='INT'
                  GROUP BY BULAN_DEPARTURE ORDER BY BULAN_DEPARTURE
                "
              );
              oci_execute($query2);
        ...
        data: 
            ...
            <?php 
              $query4 = oci_parse($koneksi,     
                ...
                WHERE TAHUN=$tahun_ini AND BULAN<=$bulan AND TYPE='INT' AND SATUAN='TEUS' ORDER BY BULAN ASC
                "
              );
              oci_execute($query4);
