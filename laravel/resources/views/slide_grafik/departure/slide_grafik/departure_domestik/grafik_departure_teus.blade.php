        ...
        <div class="card">
          <div class="card-header">
            <figure class="highcharts-figure">
              <div id="grafik_departure_teus_domestik"></div>
            </figure>
          </div>
        </div>
      </div>
...
Highcharts.chart('grafik_departure_teus_domestik', {
    chart: {
        type: 'column',
        options3d: {
          enabled: true,
    ...
    title: {
        text: 
        'Grafik Departure TEUS Domestik' 
    },
    subtitle: {
            ...
            <?php 
              $query2 = oci_parse($koneksi,     
                ...
                SELECT SUM(JML_TEUS) AS JML_TEUS, BULAN_DEPARTURE FROM DASHBOARDGRAFIK.S_PROD_PEND_PERCUSTOMER 
                  WHERE TAHUN=$tahun_lalu AND BULAN_DEPARTURE<=$bulan AND LOKASI='DOM'
        ...
        data:
            ...
            <?php 
              $query2 = oci_parse($koneksi,     
                " 
                SELECT SUM(JML_TEUS) AS JML_TEUS, BULAN_DEPARTURE FROM DASHBOARDGRAFIK.S_PROD_PEND_PERCUSTOMER 
                  WHERE TAHUN=$tahun_ini AND BULAN_DEPARTURE<=$bulan AND LOKASI='DOM'
                  GROUP BY BULAN_DEPARTURE ORDER BY BULAN_DEPARTURE
                "
              );
              oci_execute($query2);
        ...
        data: 
            ...
            <?php 
              $query4 = oci_parse($koneksi,     
                ...
                WHERE TAHUN=$tahun_ini AND BULAN<=$bulan AND TYPE='DOM' AND SATUAN='TEUS' ORDER BY BULAN ASC
                "
              );
              oci_execute($query4);
