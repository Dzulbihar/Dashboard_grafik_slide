        ...
        <div class="card">
          <div class="card-header">
            <figure class="highcharts-figure">
              <div id="line_grafik_box_total"></div>
            </figure>
          </div>
        </div>
      </div>
...
Highcharts.chart('line_grafik_box_total', {
    chart: {
        type: 'spline'
    },
    title: {
        text: 'Grafik BOX Domestik & International'
    },
    subtitle: {
          ...
          data:
            ...
            <?php 
              $query2 = oci_parse($koneksi,     
                " 
                SELECT SUM(JML_BOX) AS JML_BOX, BULAN FROM DASHBOARDGRAFIK.S_PROD_PEND_PERBULAN WHERE TAHUN=$tahun_lalu AND BULAN<=$bulan GROUP BY BULAN ORDER BY BULAN
                "
              );
              oci_execute($query2);
          ...
          data: 
            ...
            <?php 
              $query4 = oci_parse($koneksi,     
                " 
                SELECT SUM(TARGET_RKAP) AS TARGET_RKAP, BULAN FROM DASHBOARDGRAFIK.S_TARGET_RKAP_PERBULAN WHERE TAHUN=$tahun_ini AND BULAN<=$bulan AND SATUAN='BOX' GROUP BY BULAN ORDER BY BULAN
                "
              );
              oci_execute($query4);
